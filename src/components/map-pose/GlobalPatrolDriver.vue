<script setup>
/**
 * 全局巡检执行器：登录后常驻，不依赖「实时监控」页。
 * 通过 RosClient 单例发 /goal_pose、听 /nav_state；地图绘制可选。
 */
...
async function ensureChargePoint () {
  ...
  const mapId = activeMapId.value || mission.mapId
  if (!mapId) {
...
function resolveStart () {
  ...
  if (live && Number.isFinite(live.x) && Number.isFinite(live.y)) {
    const nearOrigin = Math.hypot(live.x, live.y) < 0.2
    // 无地图中心可校正时仍信任位姿；近原点时优先充电点
    if (!nearOrigin) return live
  }
  const c = chargePoint.value
...
async function claimOrphanedRunning () {
  ...
  if (!mapId) return
  try {
    let runs = await listPatrolRuns({ mapId, status: 'running' })
    let orphan = (runs || []).find((r) => r?.id && r.id !== startedRunId)
    if (!orphan) {
      runs = await listPatrolRuns({ mapId, status: 'waiting' })
...
watch(
  () => [mission.pending, mission.runId, enabled.value],
